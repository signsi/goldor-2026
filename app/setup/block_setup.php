...
if (!function_exists('rocket_ajax_load_more_events')):
    function rocket_ajax_load_more_events() {
        $args = json_decode(wp_unslash($_POST['json_data']));
        $query_args = (array) $args->{'query_args'};
        $block_args = (array) $args->{'block_args'};
        ...
        if ($ajax_query->have_posts()):
            while ($ajax_query->have_posts()): $ajax_query->the_post();
                ...
                $blade_path = 'blocks/rocketpager-events/rocketpager-events-element';
                echo \Roots\view($blade_path, $block_args)->render();
    ...
    add_action('wp_ajax_nopriv_rocket_ajax_load_more_events', 'rocket_ajax_load_more_events');
    add_action('wp_ajax_rocket_ajax_load_more_events', 'rocket_ajax_load_more_events');
endif;
