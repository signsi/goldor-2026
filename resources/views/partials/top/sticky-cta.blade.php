...
@if ($fixed_cta_active)
    <div class="sticky-cta hidden lg:block right-0 bottom-auto top-1/2 -translate-y-[50%] w-auto z-20 fixed transition-all">
        <div class="sticky-cta-inner">
            @if ($phone_active)
                <a class="flex mb-4 shadow-md bg-white text-primary h-[60px] -ml-[80px] -right-full relative rounded-l-full outline-none hover:no-underline hover:cursor-pointer hover:right-0 transition-all" href="tel:{{ $tel }}" target="_blank">
                    <div class="p-4 font-base flex justify-center items-center">
                        <svg class="w-auto h-7" width="40" height="39" viewBox="0 0 40 39" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex items-center py-0 pl-5 pr-10 h-[60px]">
                        <span class="text-xl hover:text-font transition-colors">{{ $tel }}</span>
                    </div>
                </a>
            @endif
            @if ($search_active)
                <a class="flex mb-4 shadow-md bg-white text-primary h-[60px] -ml-[80px] -right-full relative rounded-l-full outline-none hover:no-underline hover:cursor-pointer hover:right-0 transition-all">
                    <div class="p-4 font-base flex justify-center items-center">
                        <svg class="w-auto h-7" width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex items-center py-0 pl-5 pr-10 h-[60px]">
                        <form class="searchform" role="search" method="get" action="{{ esc_url( home_url( '/' ) ) }}">
                            ...
                            <div class="relative">
                                <input id="search" name="s" class="placeholder-primary shadow-none block w-full border-0 bg-transparent py-3 pl-6 leading-5 focus:outline-none ring-0 focus:ring-0 text-sm xl:text-lg" placeholder="Wonach suchen Sie?" type="search">
                            </div>
                        </form>
                    </div>
                </a>
            @endif
            @if ($link_active)
                <a class="flex mb-4 shadow-md bg-white text-primary h-[60px] -ml-[80px] -right-full relative rounded-l-full outline-none hover:no-underline hover:cursor-pointer hover:right-0 transition-all" href="{!! $link_url !!}" target="_blank">
                    <div class="p-4 font-base flex justify-center items-center">
                        <svg class="w-auto h-7" width="46" height="36" viewBox="0 0 46 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex items-center py-0 pl-5 pr-10 h-[60px]">
                        <span class="text-xl hover:text-font transition-colors">{!! $link_text !!}</span>
                    </div>
                </a>
            @endif
            @if ($scroll_active)
                <a id="to-top-button" class="flex mt-20 h-[60px] -ml-[80px] -right-full relative cursor-pointer !opacity-100 group">
