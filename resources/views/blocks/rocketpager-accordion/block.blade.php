            ...
            @while (block_rows('element'))
                ...
                @if(!block_sub_value('hide-element'))
                    <div class="accordion-item bg-white border border-gray-200">
                        <h2 class="accordion-header m-0 p-0" id="heading-{{ $identifier }}">
                        <button class="accordion-button {{ App\getFirstAccordionItemActive(block_row_index())['collClass'] }} relative flex items-center w-full py-4 px-5 text-base text-gray-800 text-left bg-white border-0 rounded-none transition focus:outline-none"
