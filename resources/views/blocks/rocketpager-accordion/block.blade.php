@php
    ...
    $button_color = 'has-' . block_value('color-button') . '-background-color has-background';
@endphp
        ...
        <div class="accordion accordion-flush" id="{{ $accordion_id }}">
            @while (block_rows('element'))
                ...
                @if(!block_sub_value('hide-element'))
                    <div class="accordion-item {{ $button_color }} border border-x-0 rounded-none">
                        <h4 class="accordion-header mb-0 pb-0" id="heading-{{ $identifier }}">
                        <button class="accordion-button {{ App\getFirstAccordionItemActive(block_row_index())['collClass'] }} relative px-gutter py-3 flex items-center w-full text-left border-0 rounded-none shadow-none transition focus:outline-none
