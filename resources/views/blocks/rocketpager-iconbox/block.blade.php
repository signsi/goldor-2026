...
@section('content-section')
    <div class="grid gap-gutter {{ $row_per_col }} @apply mb-12">
        @while (block_rows('icon'))
            ...
            @endphp
            @if (block_sub_value('link')) <a href="{{ block_sub_value('link') }}" @else <div @endif class="bg-secondary border-3 border-secondary hover:bg-white hover:border-primary flex flex-col justify-between group p-5 text-center md:text-left no-underline">
                    ...
                    <div class="{{ App\existsReturnKey('rounded-bg', 'rounded-full bg-white') }}">
                        @if ( $isImage )
                            ...
                            [
                                ...
                                'additionalClasses' => array('class' => 'transition-transform duration-300 ease-in-out mb-0 w-auto h-[75px] lg:h-[80px] mx-auto md:mx-0'),
