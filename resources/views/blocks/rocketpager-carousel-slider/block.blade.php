            ...
            <div class="carousel-element @if($isBoxSlider) p-0 pb-4 md:p-4 md:pt-0 @endif" data-carousel-id="{{ wp_rand(0, PHP_INT_MAX) }}">
                ...
                <div class="text-wrapper pr-4  {{ $isBoxSlider ? 'px-0 text-left' : 'py-4 text-left'}}">
                    ...
                    @if ( block_sub_value('email') )
                        <p><a href="mailto:{{ block_sub_value('email') }}" class="underline text-primary hover:text-font">{{ App\pl__('E-Mail') }}</a>@if ( block_sub_value('phone') ) | <a href="tel:{{ block_sub_value('phone') }}" class="underline text-primary hover:text-font">{{ block_sub_value('phone') }}</a> @endif </p>
                    @endif
                </div>
            </div>
        @endwhile
