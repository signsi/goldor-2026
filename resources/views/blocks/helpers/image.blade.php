...
@include('blocks.helpers.image',
    [
        ...
        'additionalClasses' => array('class' => 'mr-medium'),           -> (optional) zusätzlich Klassen für das Bild
    ...
    Enthält das RocketPager-xl ein Feld mit dem Namen "figcaption", so wird Unterhalb des Bildes eine Bildunterschrift ausgegeben.
--}}
...
@if( $attachment_id )
    @if($identifierLightbox)
        <a href='{{ wp_get_attachment_image_url($attachment_id, 'full') }}' data-fancybox="{{ $identifierLightbox }}" data-caption="{{ $figcaption ? $figcaption : get_the_title($attachment_id) }}" aria-label="{{ App\pl__('Bild in Lightbox anschauen') }}">
    @endif
